...
int main(int argc, char **argv)
{
    ...
    name.append(argv[2]);
    name.append(".jpg");
    Image imgx(name.c_str());
    Features_manager fm2(imgx);
    std::cout << "Focal length" << fm2.get_manhattan_world().get_focal_length() << std::endl;
    ...
    Image imggg(image);
    edges->randomly_color(imggg);
    imggg.write("./faus.jpg");
    ...
    double outlier_threshold = 0.3; //0.3
    ...
    if(fm2.get_manhattan_world().get_focal_length() > 150)
    {
        fm.set_manhattan_focal_length(fm2.get_manhattan_world().get_focal_length());
    }
    else
    {
        fm.set_manhattan_focal_length(150);
    }
