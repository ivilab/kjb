...
/* $Id: intersect_3D_hulls.c 21491 2017-07-20 13:19:02Z kobus $ */
    ...
    for (i=0; i<num_tries; i++)
    {
        ...
        for (v=0; v<dual_hp->vertex_mp->num_rows; v++)
        {
            ...
            else if ((inside == TRUE) && (original_inside == FALSE))
            {
                ...
                if ((distance < -SMALL_TOLERENCE) || (distance > TOLERANCE))
                {
        ...
        if (c1 > 15)
        {
            verbose_pso(1, "%d points for which original is incorrectly outside by at least %e!\n",
                 c1, TOLERANCE);
        }

        if (c2 > 15)
        {
            verbose_pso(1, "%d points for which original is incorrectly inside by at least %e!\n",
                 c2, TOLERANCE);
        }

        if (c3 > 15)
        {
            verbose_pso(1, "%d points for which approximation is incorrectly outside by at least %e!\n",
                 c3, TOLERANCE);
        }

        if (c4 > 15)
        {
            verbose_pso(1, "%d points for which approximation is incorrectly inside by at least %e!\n",
                 c4, TOLERANCE);
        }

        if (c5 > 15)
        {
            verbose_pso(1, "%d points for which dual is incorrectly outside by at least %e!\n",
                 c5, TOLERANCE);
        }

        if (c6 > 15)
        {
            verbose_pso(1, "%d points for which dual is incorrectly inside by at least %e!\n",
                 c6, TOLERANCE);
        }
