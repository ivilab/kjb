...
int main(int argc, char** argv)
{
    ...
    // f(x) = 2x
    x.set(2.0);
    G = gradient_cfd(two_x, x, vector<double>(1, dx));
    ...
    // f(x) = x^3
    G = gradient_cfd(x_cubed, x, vector<double>(1, dx));
    ...
    G = gradient_cfd(x_squared, x, vector<double>(D, dx));
