...
void generate_edge_sprite()
{
    ...
    // generate edge image
    edge_img_gray = Matrix(height, width, 255.0);
...
{
    Glut::set_display_mode(GLUT_LUMINANCE | GLUT_DOUBLE | GLUT_DEPTH);
    Glut_window wnd;
...
// CREATE EDGE IMAGE SPRITE
    in_sprite = Sprite(Texture().set_mask_1f(in_img_gray));
...
//    distance_map.display("distances");
    ...
    distance_sprite = Sprite(Texture().set_float(distance_map));
