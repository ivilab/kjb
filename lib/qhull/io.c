...
void qh_facet2point(facetT *facet, pointT **point0, pointT **point1, realT *mindist) {
  ...
  if (facet->toporient ^ qh_ORIENTclock) {
    vertex0= (vertexT*)SETfirst_(facet->vertices);
    vertex1= (vertexT*)SETsecond_(facet->vertices);
  }else {
    vertex1= (vertexT*)SETfirst_(facet->vertices);
    vertex0= (vertexT*)SETsecond_(facet->vertices);
  }
  zadd_(Zdistio, 2);
  ...
  && (!facet->visible || !qh NEWfacets)) {
    ...
    FOREACHridge_(facet->ridges) {
      ...
      if (neighbor->visitid != qh visit_id) {
        ...
        if (qh PRINTridges) {
          vertexA= (vertexT*)SETfirst_(ridge->vertices);
          vertexB= (vertexT*)SETsecond_(ridge->vertices);
  ...
  && (!facet->visible || !qh NEWfacets)) {
	...
	vertices= qh_setnew_delnthsorted (facet->vertices, qh hull_dim,
	   ...
	   qh_printhyperplaneintersection(fp, facet, neighbor, vertices, black);
        if (qh PRINTridges) {
          vertexA= (vertexT*)SETfirst_(vertices);
          vertexB= (vertexT*)SETsecond_(vertices);
          qh_printline3geom (fp, vertexA->point, vertexB->point, green);
        }
	qh_setfree(&vertices);
      }
...
void qh_printfacetridges(FILE *fp, facetT *facet) {
  ...
  if (facet->visible && qh NEWfacets) {
    ...
    FOREACHridge_(facet->ridges)
      ridge->seen= False;
    if (qh hull_dim == 3) {
      ridge= firstridge= (ridgeT*)SETfirst_(facet->ridges);
