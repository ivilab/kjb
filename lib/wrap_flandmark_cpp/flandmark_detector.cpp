...
int flandmark_get_normalized_image_frame(IplImage *input, const int bbox[], double *bb, uint8_t *face_img, FLANDMARK_Model *model)
{
    ...
    assert(bbox[0] >= 0 && bbox[1] >= 0 && bbox[2] < input->width && bbox[3] < input->height);
	// extend bbox by bw_margin
	d[0] = bbox[2]-bbox[0]+1; // width
    d[1] = bbox[3]-bbox[1]+1; // height
	c[0] = (bbox[2]+bbox[0])/2.0f; //center x
    c[1] = (bbox[3]+bbox[1])/2.0f; //center y
	nd[0] = d[0]*model->data.options.bw_margin[0]/100.0f + d[0]; //normalized width
	nd[1] = d[1]*model->data.options.bw_margin[1]/100.0f + d[1]; //normalized height

    bb[0] = std::max(0.0, (c[0] - nd[0]/2.0f)); //top-left x
    bb[1] = std::max(0.0, (c[1] - nd[1]/2.0f)); //top-left y
    bb[2] = std::min((double)input->width - 1, (c[0] + nd[0]/2.0f)); //bottom-right x
    bb[3] = std::min((double)input->height - 1, (c[1] + nd[1]/2.0f)); //bottom-right y
...
std::vector<double> flandmark_detect
(
    const Image& img, 
...
{
    cv::Ptr<IplImage> cv_img = opencv::to_opencv_gray(img);  
    int w = box.get_width();
    int h = box.get_height();
    int x = box.get_center()[0];
    int y = box.get_center()[1];
