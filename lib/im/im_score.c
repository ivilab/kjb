...
static int fs_current_score = 0;
static int fs_done_for_now = FALSE;
static char fs_prefix[ 1000 ];
      ...
      case KeyPress:
      {
        ...
        */
        ...
        if (event.xkey.window == windows->image.id)
        {
          ...
          // Kobus
          //
          // For now, we only allow going forward. Perhaps later we should,
          // instead of writing to a file, make a table which is output at the
          // end. Then we could go back and forward, and correct the score.
          */
...
)
{
  ...
  if ((key_symbol >= XK_0) && (key_symbol <= XK_9))
  {
      fs_current_score = key_symbol - XK_0;
  }

  /*
  else if ((key_symbol == XK_n) || (key_symbol == XK_space) || (key_symbol == XK_Return))
  {
      score = fs_current_score;
  }
...
int im_score(Image* image, ImageInfo* image_info_ptr)
{
  int argc;
  char *argv[ 1 ];
/* END Kobus */
...
)
{
    ...
    */
    ...
    if (fs_horizontal_trace) {
        im_do_horizontal_trace((*image_ptr)->rows, (*image_ptr)->columns,
                               pure_pixels, x, y);
       }

    if (fs_vertical_trace) {
        im_do_vertical_trace((*image_ptr)->rows, (*image_ptr)->columns,
                             pure_pixels, x, y);
       }
   }
  ...
  */
  ...
  while (1)
  {
      ...
      char* line_pos;
        ...
        result = BUFF_GET_REAL_LINE(stdin, line);
        ...
        (void)strcpy(image_info.filename, token);
        ...
        {
            EPETE(ss1i(token, &current_score));
            fs_current_score = current_score;
        }
...
int im_score_main(int dummy_argc, char** dummy_argv)
{
   set_error("Routine im_score_main() needs to be compiled with X11");
   return ERROR;
}
