...
/*prior parameters: P1 = generating nt P2 = generating word, tag*/
...
void add_dependency_counts(int wm,int tm,int cm,int wh,int th,int p,int ch,int dist,int subcat,int cc,int wcc,int tcc,int punc,int wpunc,int tpunc,hash_table *hash, bool shared)
{
    // wm : modifier word part of event
    // cm : constituent part 
    // wh : head word
    // th : head tag
    // p  : parent constituent
    // ch : head constituent
    // dist: distance feature
    // subcat: subcategorization frame
    // cc, wcc, tcc : conjunction flag, word and tag
    // punc, wpunc, tpunc: punctuation flag, word and tag

    unsigned char buffer[1000]; // create a buffer to hold keys for counting event types

    // 
    if(cm != get_stop_nt()) add_prior_counts(cm,wm,tm,hash, shared); 

    wm = fwords[wm];
    p  = argmap[p]; // convert these to lookup values
    ch = gapmap[ch];

    // create a dependency string for the buffer
    make_dep1_string(buffer,wm,tm,cm,wh,th,p,ch,dist,subcat,cc,punc);
    // add counts to the hash database
    add_counts(buffer,D1OLEN,D1BACKOFFS,D1PROBTYPE,hash, shared);

    // similarly for second type of dependency string
    make_dep2_string(buffer,wm,tm,cm,wh,th,p,ch,dist,subcat,cc,punc);
    add_counts(buffer,D2OLEN,D2BACKOFFS_NOTAG,D2PROBTYPE,hash, shared);

    if(cc==1) // if there is a conjunction, track counts for conjunction probs
    {
	make_pcc1_string(buffer,tcc,p,ch,th,wh,cm,tm,wm,0);
	add_counts(buffer,CP1OLEN,CP1BACKOFFS,CP1PROBTYPE,hash, shared);

	make_pcc2_string(buffer,wcc,tcc,p,ch,th,wh,cm,tm,wm,0);
	add_counts(buffer,CP2OLEN,CP2BACKOFFS_NOTAG,CP2PROBTYPE,hash, shared);
    }

    if(punc==1) // similarly for punctuation
    {
...
double get_dependency_prob(int wm,int tm,int cm,int wh,int th,int p,int ch,int dist,int subcat,int cc,int wcc,int tcc,int punc,int wpunc,int tpunc,hash_table *hash)
{
    ...
    wm = fwords[wm];
    p  = argmap[p]; // convert args to lookup keys
    ch = gapmap[ch];

    // create a dependency string for args provided
    make_dep1_string(buffer,wm,tm,cm,wh,th,p,ch,dist,subcat,cc,punc);
    // compute smoothed probability from function in genprob.cc
    // 0 and 5 are hardcoded smoothing coefficients (as described in Bikel)
    p1 = get_prob(buffer,D1OLEN,D1BACKOFFS,D1PROBTYPE,0,5,hash);

    if(cm != get_stop_nt())
    {   // unless we are at a stop, there is a word probability as well
	// (if at a stop, the probability of generating #STOP# is 1)
	make_dep2_string(buffer,wm,tm,cm,wh,th,p,ch,dist,subcat,cc,punc);
    ...
    if(cc == 1)
    {   // if there is an intervening conjunction, need its probability
	make_pcc1_string(buffer,tcc,p,ch,th,wh,cm,tm,wm,0);
...
/*  printf("DEP %d %d %d %d %d %d %d %d %d %d %d %g %g %g\n",
    ...
    // the full dependency probability is the product of these
    // four factors (where p3 and p4 are themselves products of two terms
    // if there is a conjunction and punctuation, respectively)
    return log(p1*p2*p3*p4);
}

void add_prior_counts(int ch,int wh,int th,hash_table *hash, bool shared)
{
    unsigned char buffer[1000];
    ...
    add_counts( buffer, P1OLEN, P1BACKOFFS, P1PROBTYPE, hash, shared );
  
    make_prior2_string( buffer, ch, wh, th );
    add_counts(buffer,P2OLEN,P2BACKOFFS,P2PROBTYPE,hash, shared );
}
...
void add_unary_counts(int ch,int wh,int th,int p,hash_table *hash, bool shared)
{
    unsigned char buffer[1000];

    add_prior_counts( ch, wh, th, hash, shared );
	...
	add_counts(buffer,S1OLEN,S1BACKOFFS,S1PROBTYPE,hash, shared);

	make_s2_string(buffer,ch,wh,th,p);
	add_counts(buffer,S2OLEN,S2BACKOFFS_NOTAG,S2PROBTYPE,hash, shared);
    }
    else
    {
	make_unary_string(buffer,ch,wh,th,p);
	add_counts(buffer,UOLEN,UBACKOFFS,UPROBTYPE,hash, shared);
    }
}
...
void add_subcat_counts(int subcat,int ch,int wh,int th,int p,int dir,hash_table *hash, bool shared)
{
  unsigned char buffer[1000];
  ...
  add_counts(buffer,SCOLEN,SCBACKOFFS,SCPROBTYPE,hash, shared);
...
void add_gap_counts(int gap,int ch,int wh,int th,int p,hash_table *hash, bool shared)
{
  unsigned char buffer[1000];
  ...
  add_counts(buffer,GOLEN,GBACKOFFS,GPROBTYPE,hash, shared);
...
/*make string for P(cm,t,punc,cc | context) 
    ...
    int            punc)
{
  ...
  //context for kth smoothing level
  string[7] = (char) p;
  string[8] = (char) ch;
  string[9] = (char) newdist;
  byte3_to_char(&string[10],subcat);

  //add'l context for (k-1)th level
  string[13] = (char) th;

  //add'l context for (k-2)th level
...
void add_tagword_counts(int wm,int tm,hash_table *hash, bool shared)
{
  unsigned char buffer[1000];
  int bos[] = {1,1};
  ...
  add_counts_level(buffer,D2OLEN,bos,3,D2PROBTYPE,hash, shared);
  add_counts_level(buffer,S2OLEN,bos,2,S2PROBTYPE,hash, shared);
  add_counts_level(buffer,CP2OLEN,bos,3,CP2PROBTYPE,hash, shared);
}
...
void make_s1_string(unsigned char *string,int ch,int wh,int th,int p)
{
    ...
    //context for only smoothing level
    string[5] = (char) p;
}
