...
// If true, print output parse tree in Collins format
int PRINT_EDGES;
...
void init_saments()
{
    ...
    for(i=0; i<GMAXNTS; i++) saments[i].edge1 = -1;
}
...
int add_edge(int s,int e)
{
    ...
    */
    ...
    {
	if(edges[numedges].prob < pthresh) return -1;
    }
    ...
    {
        /* don't count POS tags in beam comparisons*/
	if(edges[numedges].type == 0)
	{
	    bestprobs[s][e] = -10000;
	} else {
	  ...
	  spanning words s..e*/
    } else if(is_top(edges[numedges].label)) {
      /*NT_TOP can only span the whole sentence (it's the start symbol)*/
      if(s!=0 || e!=current->nws_np-1) return -1;
    } else {
	...
	if(edges[numedges].type!=0&&!inbeam(numedges,s,e))
	{
	    ...
	    edges[numedges].prob2 > bestprobs[s][e]
  ...
  if(saments[nt].edge1==-1)
  {
	...
	sindex[s][e]*/
      if(sindex[s][e]==-1) sindex[s][e] = numedges;
...
void add_singles(int s,int e,int si,int ei)
{
  ...
  for(i=si;i<=ei;i++)
    if(edges[i].valid==1&&inbeam(i,s,e)&&edges[i].stop==1)
      for(j=0;j<unary_nums[(unsigned int) edges[i].label];j++)
	if(unaries[(unsigned int) edges[i].label][j]!=-1)
	{
	  etemp=edges[i];
	  etemp.stop=0;
	  etemp.type=1;
	  etemp.label=unaries[(unsigned int) edges[i].label][j];
	  etemp.headch=i;

	  etemp.headlabel=edges[i].label;

	  if(is_npb(etemp.label))
	    etemp.hasverb = 0;
	  ...
	  flaggap=(hasgap[(unsigned int) etemp.label] && !hasgap[(unsigned int) edges[i].label]);
...
void print_edge(int e,int off)
{
  ...
  if( is_high_np(edges[e].label)
     && edges[e].numchild ==1)
    {
      j= childs[edges[e].child1];
/*      if(is_npb(edges[j].label))
	flag=0;*/
    }
  ...
  if(flag)
    {
      printf("%s %g ",nts[(unsigned int) edges[e].label],edges[e].prob);
      newoff=off+strlen(nts[(unsigned int) edges[e].label])+1;
    }
  else
    newoff=off;
...
void print_wholent(int e)
{
  ...
  printf("%s",nts[(unsigned int) edges[e].label]);
  ...
  if(edges[e].type==0)
	...
	for(i=0;i<current->wordpos[edges[e].head];i++)
      ...
      printf("%s/%s ",current->words[current->wordpos[edges[e].head]]
	     ,nts[(unsigned int) edges[e].headtag]);
...
void join_2_edges_follow(int e1,int s,int m,int e,int *e2s,int ne2s)
{
  ...
  */
  ...
  int dist = -1,subcat = -1,cc = -1,punc = -1;
  int wcc = -1,tcc = -1,wpunc = -1,tpunc = -1;
  int wh = -1,ch = -1,th = -1;
  int wm = -1,cm = -1,tm = -1;
  int p = -1;
  int i = -1,k = -1;
...
/*ccprob is used to prefer wide scope premodification of CCed phrases,
  ...
  */
  ...
  if(edges[e1].type==1&& is_npb(edges[e1].label))
...
/*     &&tablef[edges[e1].label][e1hlabel][edges[e2].label])*/
      ...
      for(i2=0;i2<ne2s;i2++)
	{
	  ...
	  if(combineprob>=pthresh &&
	     ...
	     edges[e2].stop ==1 &&
	     tablef[(unsigned int) edges[e1].label][e1hlabel][(unsigned int) edges[e2].label])
	    {
	      h2_is_verb=edges[e2].hasverb;
	      if(e3flag2==0)
		{
		  if(! is_npb(edges[e1].label))
		    {
	      ...
	      if(edges[e1].type!=0 && argmap[(unsigned int) edges[e1].label]==argmap[(unsigned int) edges[edges[e1].headch].label])
...
/*	  printf("BBB %g\n",combineprob);
	  ...
	  if(tablep[(unsigned int) edges[e2].label][e2hlabel][(unsigned int) edges[e1].label] &&
	     combineprob>=pthresh &&
	     edges[e1].stop ==1 &&
	     (sindex[s][e]==-1 || combineprob>=(bestprobs[s][e]-BEAMPROB)))
	    {
		...
		{
		  if(edges[e2].type!=0 &&
		     argmap[(unsigned int) edges[e2].label]==argmap[(unsigned int) edges[edges[e2].headch].label])
		    ccprob_pre=0;
		  else
...
void complete(int s, int e)
{
  ...
  for(i=s;i<e;i++)
	...
	if(is_cc(current->tagnos[i+1])&&i<e-1)
	  for(j=sindex[s][i];j<=eindex[s][i];j++)
	    if(inbeam2(j,s,i))
...
int find_best_parse(double & prob)
{
  ...
  for(i = sindex[0][current -> nws_np - 1];
      i <= eindex[0][current -> nws_np - 1];
      i++)
    if(is_top(edges[i].label) && edges[i].stop==0)
      {
/*	printf("prob %g\n",edges[i].prob);
	print_edge(i,0);*/
	if(best==-1)
...
int inbeam(int edge,int s,int e)
{
  if(edges[edge].type!=1)
    ...
    if(is_high_np(edges[edge].label))
      {
...
/** 
 * Constructs one tree corresponding to this edge (or more if there is punc)
 */
static TreePtr edgeToTree(int e, 
			  list<TreePtr> & prevPunc,
			  list<TreePtr> & nextPunc) 
{
  // POS tag case
  if(edges[e].type == 0){
    ...
    if(edges[e].head==0){
      for(int i=0;i<current->wordpos[edges[e].head];i++){
	TreePtr punc(new Tree(current->words[i], current->tags[i]));
	prevPunc.push_back(punc);
      }
    }

    // this edge
    TreePtr term(new Tree(current->words[current->wordpos[edges[e].head]],
			  nts[(unsigned int) edges[e].headtag]));

    int w = current->wordpos[edges[e].head];
    int next;
    if(edges[e].head == current->nws_np - 1){
      next = current->nws;
    } else {
    ...
    for(int i = w + 1; i < next; i ++){
      TreePtr punc(new Tree(current->words[i], current->tags[i]));
      nextPunc.push_back(punc);
    }
  ...
  // non-terminal
  TreePtr tree(new Tree(nts[(unsigned int) edges[e].label]));

  for(int i = edges[e].child1; i < edges[e].child1 + edges[e].numchild; i ++){
    list<TreePtr> pp, fp;
    TreePtr child = edgeToTree(childs[i], pp, fp);

    // if this is the left-most child, propagate preceding punctuation up
    if(i == edges[e].child1){
      prevPunc = pp;
    }
    // otherwise, append previous punctuation
    else{
      for(list<TreePtr>::const_iterator it = pp.begin(); 
	  it != pp.end(); it ++){
	tree->addChild(* it);
      }
    }
    ...
    // set head
    if(childs[i] == edges[e].headch){
      tree->setHead(child);
    }

    // if this is the right-most child, propagate following punctuation up
    if(i == edges[e].child1 + edges[e].numchild - 1){
      nextPunc = fp;
    }
    // otherwise, append following punctuation
    else{
      for(list<TreePtr>::const_iterator it = fp.begin(); 
	  it != fp.end(); it ++){
	tree->addChild(* it);
      }
    }
...
{
  list<TreePtr> prevPunc, nextPunc;
  TreePtr tree = edgeToTree(edge, prevPunc, nextPunc);

  // append top-level preceding punctuation
  for(list<TreePtr>::reverse_iterator it = prevPunc.rbegin();
      it != prevPunc.rend(); it ++){
    tree->addChild(* it, false);
  }

  // append top-level following punctuation
  for(list<TreePtr>::const_iterator it = nextPunc.begin();
      it != nextPunc.end(); it ++){
    tree->addChild(* it);
  }

  // COUT << "This tree:" << endl << tree << endl;
