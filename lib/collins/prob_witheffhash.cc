...
double get_dependency_prob_witheffhash(int wm,int tm,int cm,int wh,int th,int p,int ch,int dist,int subcat,int cc,int wcc,int tcc,int punc,int wpunc,int tpunc,hash_table *hash,effhash_table *effhash)
{
  ...
  make_alldep_string((char *) buffer,wm,tm,cm,wh,th,p,ch,dist,subcat,cc,wcc,tcc,punc,wpunc,tpunc);
  buffer[0] = DEFFTYPE;
  buffer[1] = 0;
  buffer[2] = 0;
...
void make_alldep_string(char *string,int wm,int tm,int cm,int wh,int th,int p,int ch,int dist,int subcat,int cc,int wcc,int tcc,int punc,int wpunc,int tpunc)
{
  ...
  byte2_to_char((unsigned char *) &string[3],wm);
  ...
  string[10] = (char) ch;
  string[11] = (char) dist;
  byte3_to_char((unsigned char *) &string[12],subcat);
  string[15] = (char) th;
  byte2_to_char((unsigned char *) &string[16],wh);

  if(cc)
    {
      byte2_to_char((unsigned char *) &string[18],wcc);
      string[20] = (char) tcc;
    }
  else
    {
  ...
  if(punc)
    {
      byte2_to_char((unsigned char *) &string[18],wpunc);
