...
void handle_key(Scene_viewer& viewer, unsigned char key, int, int)
{
    ...
    switch(key)
    {
        ...
        case 'H':
        {
            BOOST_FOREACH(const Target& tg, scene_p->association)
            {
                if(tg.changed_start() == tg.get_start_time()
                        && tg.changed_end() == tg.get_end_time())
                {
                    tg.estimate_height(scene_p->camera);
                }
            }
            viewer.set_scene(*scene_p);
            cur_mode = ESTIMATE_HEIGHT;
            break;
        }
        ...
        case 'T':
        {
            BOOST_FOREACH(const Target& tg, scene_p->association)
            {
                        ...
                        && tg.changed_end() == tg.get_end_time())
                {
                    if(tg.changed())
                    {
                        tg.estimate_trajectory(scene_p->camera);
                    }
                }
            }
            viewer.set_scene(*scene_p);
        ...
        case 'I':
        {
            BOOST_FOREACH(const Target& tg, scene_p->association)
            {
                        ...
                        && tg.changed_end() == tg.get_end_time())
                {
                    if(tg.changed())
                    {
                        initialize_directions(tg.trajectory(), true);
                    }
                }
            }
            viewer.set_scene(*scene_p);
            cur_mode = INITIALIZE_DIRECTION;
            break;
        }
        ...
        case 'B':
        {
            BOOST_FOREACH(const Target& tg, scene_p->association)
            {
                        ...
                        && tg.changed_end() == tg.get_end_time())
                {
                    if(tg.changed())
                    {
                        tg.update_boxes(scene_p->camera);
                    }
                }
            }
            viewer.set_scene(*scene_p);
            cur_mode = UPDATE_BOXES;
            break;
        }
        ...
        case 'L':
        {
            BOOST_FOREACH(const Target& tg, scene_p->association)
            {
                        ...
                        && tg.changed_end() == tg.get_end_time())
                {
                    if(tg.changed())
                    {
                        tg.update_faces(scene_p->camera);
                    }
                }
            }
            viewer.set_scene(*scene_p);
            cur_mode = UPDATE_FACES;
            break;
        }

        case 'M':
        {
            clear_facemarks(scene_p->association);
            update_facemarks(scene_p->association, fm_data);
            viewer.set_scene(*scene_p);
            cur_mode = UPDATE_FACEMARKS;
            break;
        }
        ...
        case 'R':
        {
            BOOST_FOREACH(const Target& tg, scene_p->association)
            {
                tg.refine_trajectory(scene_p->camera);
            }
            viewer.set_scene(*scene_p);
...
void update_viewer(Scene_viewer& viewer)
{
    ...
    else if(cur_mode == ESTIMATE_HEIGHT)
    {
        info_text[1] = "Mode: EH";
    }
    else if(cur_mode == INITIALIZE_DIRECTION)
    {
        info_text[1] = "Mode: ID";
    }
    else if(cur_mode == UPDATE_BOXES)
    {
        info_text[1] = "Mode: UB";
    }
    else if(cur_mode == UPDATE_FACES)
    {
        info_text[1] = "Mode: UF";
    }
    else if(cur_mode == UPDATE_FACEMARKS)
    {
        info_text[1] = "Mode: UFM";
    }
    else if(cur_mode == ESTIMATE_TRAJECTORY)
    {
        info_text[1] = "Mode: ET";
    }
    else if(cur_mode == ESTIMATE_DIRECTION)
    {
        info_text[1] = "Mode: ED";
    }
