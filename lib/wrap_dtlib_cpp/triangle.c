...
/*
 * Kobus: Instead of making the makefiles more complex, we will just define
 * ncuts specific symbols here.
*/
#define NO_TIMER
#define REDUCED
#define SINGLE
...
#ifdef OBSOLETE_AND_BROKEN   /* Kobus. */
...
)
{
  ...
  /* Kobus: */
  /* Split((REAL)b, (REAL)bhi, (REAL)blo); */
  Split((REAL)b, bhi, blo);
...
)
{
  ...
  det = estimate(4, B);
  errbound = (REAL)ccwerrboundB * (REAL)detsum;
  ...
  errbound = (REAL)ccwerrboundC * (REAL)detsum + (REAL)resulterrbound * (REAL)Absolute(det);
...
)
{
  ...
  errbound = (REAL)iccerrboundB * (REAL)permanent;
  if ((det >= errbound) || (-det >= errbound)) {
    return det;
  }
  ...
  errbound = (REAL)iccerrboundC * (REAL)permanent + (REAL)resulterrbound * (REAL)Absolute(det);
      ...
      if (top.tri == dummytri) {
        ...
        if (doflip) {
          ...
          if (insertpoint != (point) NULL) {
            if (counterclockwise(leftpoint, insertpoint, rightpoint) < 0.0f) {
              printf("Internal error in insertsite():\n");
...
*/
            if (counterclockwise(farpoint, leftpoint, insertpoint) < 0.0f) {
              printf("Internal error in insertsite():\n");
              printf("  Clockwise triangle after edge flip (left).\n");
            }
            if (counterclockwise(insertpoint, rightpoint, farpoint) < 0.0f) {
              printf("Internal error in insertsite():\n");
...
)
{
  ...
  while (virusloop != (triangle **) NULL) {
    ...
    if (regionattrib) {
      /* Set an attribute. */
      setelemattribute(testtri, eextras, (REAL)attribute);
    }
    if (vararea) {
      /* Set an area constraint. */
      setareabound(testtri, (REAL)area);
    }
    if (verbose > 2) {
      /* Assign the triangle an orientation for convenience in */
      /*   checking its points.                                */
...
void quality_statistics()
{
  ...
  radconst = (REAL)PI / 18.0f;
  degconst = 180.0f / (REAL)PI;
  for (i = 0; i < 8; i++) {
  ...
  if (biggestangle >= 1.0f) {
    ...
    if (acutebiggest) {
      ...
      biggestangle = 180.0f - degconst * (REAL)acos(sqrt(biggestangle));
    }
  }
