...
void logger(const Parametric_parapiped & ipp, const Perspective_camera & ic, double ll)
{
	...
	(*pp) = ipp;
    (*camera) = ic;
    double _ll = likelihood->compute_likelihood<Parametric_parapiped, Perspective_camera>(*pp, *camera);
    if(_ll < best_ll)
    {
    	(*best_pp) = ipp;
    	(*best_camera) = ic;
...
static void display_glut()
{
    ...
    camera->prepare_for_rendering(true);
    pp->solid_render();
    std::cout << "GLUT pitch:" << best_camera->get_pitch() << std::endl;
    _mutex.unlock();
...
int main(int argc, char** argv)
{
	...
	pp = new Parametric_parapiped(1.0,0.0,0.0, 30.0, 20.0, 10.0);
	pp->set_yaw(0.062);
	...
	double best_ll = likelihood->compute_likelihood<Parametric_parapiped, Perspective_camera>(*pp, *camera);
	std::cout << "The likelihood is" << best_ll << std::endl;
