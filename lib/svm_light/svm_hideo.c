...
/* $Id: svm_hideo.c 4727 2009-11-16 20:53:54Z kobus $ */
...
double *optimize_qp(QP *qp, double *epsilon_crit, long int nx, double *threshold, LEARN_PARM *learn_parm)
...
int optimize_hildreth_despo(long int n, long int m, double precision, double epsilon_crit, double epsilon_a, long int maxiter, long int goal, long int smallround, double lindep_sensitivity, double *g, double *g0, double *ce, double *ce0, double *low, double *up, double *primal, double *init, double *dual, long int *lin_dependent, double *buffer, double *progress)
                          /* number of variables */
                          /* number of linear equality constraints [0,1] */
                          /* solve at least to this dual precision */
                          /* stop, if KT-Conditions approx fulfilled */
                          /* precision of alphas at bounds */
                          /* stop after this many iterations */
                          /* keep going until goal fulfilled */
                          /* use only two variables of steepest descent */
                                /* epsilon for detecting linear dependent ex */
                          /* hessian of objective */
                          /* linear part of objective */
                          /* linear equality constraints */
                          /* box constraints */
                          /* primal variables */
                          /* initial values of primal */
                          /* dual variables */
                           
                    
                          /* delta in the objective function between
                             before and after */
{
...
int solve_dual(long int n, long int m, double precision, double epsilon_crit, long int maxiter, double *g, double *g0, double *ce, double *ce0, double *low, double *up, double *primal, double *d, double *d0, double *ig, double *dual, double *dual_old, double *temp, long int goal)
     ...
     /* equality constraints. */
                          ...
                          /* solve at least to this dual precision */
                          /* stop, if KT-Conditions approx fulfilled */
                          /* stop after that many iterations */
...
void linvert_matrix(double *matrix, long int depth, double *inverse, double lindep_sensitivity, long int *lin_dependent)
...
void lprint_matrix(double *matrix, long int depth)
{
  long i,j;
  for(i=0;i<depth;i++) {
    for(j=0;j<depth;j++) {
...
void ladd_matrix(double *matrix, long int depth, double scalar)
{
  long i,j;
  for(i=0;i<depth;i++) {
    for(j=0;j<depth;j++) {
...
void lcopy_matrix(double *matrix, long int depth, double *matrix2)
{
  long i;
...
void lswitch_rows_matrix(double *matrix, long int depth, long int r1, long int r2)
{
  long i;
  double temp;
...
void lswitchrk_matrix(double *matrix, long int depth, long int rk1, long int rk2)
{
  long i;
  double temp;
...
double calculate_qp_objective(long int opt_n, double *opt_g, double *opt_g0, double *alpha)
{
  double obj;
  long i,j;
